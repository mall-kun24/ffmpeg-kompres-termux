...
# Bagian preset pilihan
echo -e "\nPilih preset FFmpeg:"
echo "1. ultrafast   (paling cepat tapi kualitas rendah)"
echo "2. superfast   (sangat cepat, kualitas agak rendah)"
echo "3. veryfast    (cepat, kualitas cukup baik) ✅ default"
echo "4. faster      (sedikit lebih lambat, kualitas oke)"
echo "5. fast        (keseimbangan bagus)"
echo "6. medium      (standar, kualitas lumayan tinggi)"
echo "7. slow        (lebih lambat, kualitas tinggi)"
echo "8. slower      (paling lambat, kualitas sangat tinggi)"
read -p "Pilihan preset (1-8): " pnum
...
echo "🔄 Mulai kompresi..."
ffmpeg -i "$input" -vf "scale=$res,fps=$fps" -c:v $codec -preset $preset -b:v $bitrate $extra_flags -c:a copy "$output" -y -loglevel error
...
if [ $? -eq 0 ]; then
    ...
    read -p "Yakin ingin replace? [ya/tidak]: " replace
    if [[ "$replace" == "ya" ]]; then
        mv -f "$output" "$input"
